/* 
* PIA code, mon 15/04/2024, due tomorrow.

* LCD screen with bitmap images stored on SD. 
* Access image filename from an array, show on command.

* Using counter & random function.
* Using ST7735 TFT driver and Spi.
*/

// libraries:
#include <Adafruit_GFX.h>     // graphics library
#include <Adafruit_ST7735.h>  // library for driver st7735
#include <SPI.h>              // SPI library: controlling multiple devices
#include <SD.h>               // SD library: using images stored on SD

// defining screen connector pins
...
int count = 0;  // incremements per change in little screen text.

long maxInterval = 11;     // range of 0 to 10!
long currentInterval = 0;  // random interval, between 1 & 10 movements of bar.

void setup() {
  Serial.begin(9600);
  pinMode(TFT_CS, OUTPUT);
  digitalWrite(TFT_CS, HIGH);  // display data on screen
  tft.initR(INITR_BLACKTAB);

  Serial.print("Initializing SD card...");
  if (!SD.begin(SD_CS)) {
    Serial.println("she flop!!!");
    while (1)
      ;  // freeze code infinite loop
  }
  Serial.println("She ate");
...
void loop() {
  ...
  while (true) {
    ...
    getNewImg(entry);
    entry.close();
    int randomDelay = (int)random(3000, 5000);
